...
class social_stgcnn(nn.Module):
    def __init__(self,n_stgcnn =1,n_txpcnn=5,input_feat=2,output_feat=5,  #input_feat is vx and vy should be changed!!! to 21 maybe

                                                                  ##### paramter if assigned from calling they take the values from calling and if not assigned they take default here.
    ...
    def calculate_paramter_constant_velocity(self,v,mask,mean_x,mean_y):
        ...
        v[0, :, mask, 0] = mean_x[mask]
        v[0, :, mask, 1] =mean_y[mask]
       # print(v[0,2,:,mask])
    ...
    def forward(self,v,a):
        velocities_mean_x_y = v[0, :2, :, :].mean(dim=1)
        mask = (torch.abs(velocities_mean_x_y[0, :]) == 0) & (torch.abs(velocities_mean_x_y[1, :]) == 0) #stationry if vecloitry is mean velocity is smaller than 0.01 m in 0.4 second
       # mask = condition.view(1, -1)
        mean_x, mean_y=v[0,:2,-1,:]
